...
``` text
Customershoppingbehaviour\_analysis/
│
├── CustomerShoppingBehavior.csv
├── CustomerShoppingDB.sql
├── Data\_Analysis.ipynb
├── PowerBI\_CustomerShoppingBehavior\_Dashboard.pbix
└── README.md
```
...
3. Renaming `purchase\_amount\_(usd)` to `purchase\_amount`
...
The check returns true, so the notebook removes `promo\_code\_used` from
...
The notebook `Data\_Analysis.ipynb` follows this sequence:
...
``` python
df = pd.read\_csv("CustomerShoppingBehavior.csv")
```
...
and removes the redundant `promo\_code\_used` field.
